...
> Контроллер может и НЕ наследоваться от абстрактного контроллера, однако в таком случае не будет доступа к методам абстрактного контроллера (кто бы мог подумать)
...
Можно указать обратную ситуацию и указать метод приватным (хотя по-умолчанию он и так приватный). Оба метода также могут принимать булиан параметр для динамического изменения видимости маршрута - например, в зависимости от прав пользователя

```php
Route::post('action_name', /** замыкание */)->isPublic(is_admin()); // доступен только администраторам, например
Route::post('action_2_name', /** замыкание */)->isPrivate(false);
```
